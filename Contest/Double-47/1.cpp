...
Cartesian grid: (x, y). You are also given an array points where each points[i] = [ai,
bi] represents that a point exists at (ai, bi). A point is valid if it shares the same
...
The Manhattan distance between two points (x1, y1) and (x2, y2) is abs(x1 - x2) + abs(y1
- y2)
