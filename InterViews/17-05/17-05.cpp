...
number of letters and numbers. Return the subarray. If there are more than one answer,
return the one which has the smallest index of its left endpoint. If there is no answer,
return an empty arrary.
*/
/*
前缀和 + 哈希表
1. 字符看作 1 ，数字看作 -1，即求最长的子数组，使得和为 0
2. maxL 和 startI 分别记录满足条件的子数组的长度和左端点的位置；
-- 若当前位置前缀和 sum 在哈希表中存在，记录 s 第一次出现的位置为 j ，则 [j+1, i]
的子数组和为 0，若 max < i - j, 更新，startI = j + 1
