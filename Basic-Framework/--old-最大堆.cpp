 ...
 * MaxHeap构造函数
 */
template <class T> MaxHeap<T>::MaxHeap() { new (this) MaxHeap(DEFAULT_SIZE); }

template <class T> MaxHeap<T>::MaxHeap(int capcaity) {
	mSize = 0;
	mCapacity = capcaity;
 ...
 *       不存在 —— 返回 -1
 */
template <class T> int MaxHeap<T>::getIndex(T data) {
 ...
 */
template <class T> void MaxHeap<T>::filterUp(int start) {
	int c = start;       //当前节点下标
	int p = (c - 1) / 2; //父亲节点下标
 ...
 *   -1 —— 插入失败
 */
template <class T> int MaxHeap<T>::insert(T data) {
	//堆满
	if (mSize == mCapacity)
		return -1;
 ...
 *
 */
template <class T> void MaxHeap<T>::filterDown(int start, int end) {
	int c = start;     //当前节点的下标
	int l = 2 * c + 1; //左子节点下标
 ...
 *   -1 —— 失败
 */
template <class T> int MaxHeap<T>::remove(T data) {
	//堆空不需要删除
	if (mSize == 0)
		return -1;
