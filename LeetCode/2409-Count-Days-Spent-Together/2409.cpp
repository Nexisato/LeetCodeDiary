...
public:
	int countDaysTogether(string arriveAlice, string leaveAlice, string arriveBob,
		...
		int arriveA = calcDayOfYear(arriveAlice, prefixSum),
		    leaveA = calcDayOfYear(leaveAlice, prefixSum);
		int arriveB = calcDayOfYear(arriveBob, prefixSum),
		    leaveB = calcDayOfYear(leaveBob, prefixSum);
