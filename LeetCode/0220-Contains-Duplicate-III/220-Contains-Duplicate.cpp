...
are two distinct indices i and j in the array such that abs(nums[i] - nums[j]) <= t and
abs(i - j) <= k.
...
2. 利用set快速crud元素。对于元素x，当我们希望判断滑动窗口中是否存在某个数y落在区间[x -
t, x + t]中， 只需要判断滑动窗口中所有大于等于x - t的元素中最小元素是否小于等于x + t即可
*/
class Solution {
public:
