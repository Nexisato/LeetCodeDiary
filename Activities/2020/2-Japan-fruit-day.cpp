...
Description:
日本每月的 22 日是水果酥饼日。因为看日历的时候，22 日的上方刚好是 15 日，也就是“‘22’
这个数字上面点缀着草莓”[1]
...
notes:如果将日语的 15 拆为 1 和 5
发音，则与日语“草莓”一词发音相同，而水果酥饼中最为著名的就是草莓酥饼。 同时，日历中 15
总在 22 的上面，故有此说法。 ——译者注。
*/
#include <bits/stdc++.h>
using namespace std;
...
1. 将n个数看作无向图中的n个节点，若两个节点 n1
n2之和为完全平方数，则对应节点之间存在一条无向边
2.
问题转化为：从某一个节点开始，通过无向边遍历每个节点恰好一次，并最终返回初始节点（即哈密顿回路,NP
Problem）
